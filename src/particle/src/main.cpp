...
std::vector<int> timeUpdate;
std::vector<int> timeResample;
/*
* TODO
* Define the proper noise values
*/
double sigma_init [3] = {0.0, 0.0, 0.0};  //[x,y,theta] initialization noise. 
double sigma_pos [3]  = {0.1, 0.1, 0.1}; //[x,y,theta] movement noise. Try values between [0.5 and 0.01]
...
void PointCloudCb(const sensor_msgs::PointCloud2ConstPtr& cloud_msg){
    ...
    timeUpdate.push_back(duration.count());
    ...
    auto durationR = std::chrono::duration_cast<std::chrono::nanoseconds>(endR-startR);
    timeResample.push_back(durationR.count());
...
int main(int argc,char **argv)
{
    ...
    #ifdef RANDOM
    normal_distribution<double> index_gen(0, cloud_filtered_map->points.size()-1);
    int index = index_gen(gen);
    double x = cloud_filtered_map->points[index].x;
    double y = cloud_filtered_map->points[index].y;
    pf.init(x, y, GPS_theta, sigma_init, NPARTICLES);
    #else
    ...
    myfile.close();
    int sum = 0;
    for(auto &elem:timeUpdate){
        sum += elem; 
    }
    std::cout << "update avg: " << sum/timeUpdate.size() << std::endl;
    int sumR = 0;
    for(auto &elem:timeResample){
        sumR += elem; 
    }
    std::cout << "resample avg: " << sumR/timeResample.size() << std::endl;
